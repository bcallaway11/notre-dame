...
subtitle: "Session 4: More Complicated Treatment Regimes"
...
[Running Example:]{.alert} Causal effect of the length of school closures on student test scores
...
eqlist <- list("ATT(d|d) &= \\E[Y_{t=2}(d) - Y_{t=2}(0) | D=d] \\hspace{150pt}",
               ...
               "&= \\E[Y_{t=2}(d) - Y_{t=1}(0) | D=d] - \\E[\\Delta Y_{t=2}(0) | D=0]",
               "&= \\E[\\Delta Y_{t=2} | D=d] - \\E[\\Delta Y_{t=2} | D=0]")
...
eqlist <- list("ATE(d_h) - ATE(d_l) &= \\E[Y_{t=2}(d_h) - Y_{t=2}(0)] - \\E[Y_{t=2}(d_l) - Y_{t=2}(0)]",
              "&= \\underbrace{\\E[Y_{t=2}(d_h) - Y_{t=2}(d_l)]}_{\\textrm{Causal Response}}")
...
\end{align*}
where $m_\Delta(l) := \E[\Delta Y_{t=2}|D=l] - \E[\Delta Y_{t=2}|D=0]$ and $w(l)$ are weights
...
\begin{align*}
  ATT(\mathbf{d}_t, t) := \E[Y_{t}(\mathbf{d}_t) - Y_{t}(\mathbf{0}_t) | \mathbf{D}_{t} = \mathbf{d}_t]
\end{align*}
...
\begin{align*}
  \E[\Delta Y_{t}(\mathbf{0}_t) | \mathbf{D}_{t} = \mathbf{d}_t] = \E[\Delta Y_{t}(\mathbf{0}_t) | \mathbf{D}_{t} = \mathbf{0}_t]
\end{align*}
...
\begin{align*}
  ATT(\mathbf{d}_t, t) = \E[Y_{t} - Y_{1} | \mathbf{D}_{t} = \mathbf{d}_t] - \E[Y_{t} - Y_{1} | \mathbf{D}_{t} = \mathbf{0}_t]
\end{align*}
...
2. The previous point provides an additional reason to try to aggregate the group-time average treatment effects.  However, this is also not so straightforward.
   ...
   * This is an area of active research (e.g., @chaisemartin-dhaultfoeuille-2024 and @yanagi-2022)
...
* Formally, it amounts to averaging over all subsequent treatments decisions (@chaisemartin-dhaultfoeuille-2024)

In math: Define $M_i := \min\{t : D_{it} = 1\}$, then we can consider the (timing-group)-time average treatment effects:
$$ATT(m,t) := \E[Y_{t}(\mathbf{D}_{t}) - Y_{t}(\mathbf{0}_t) | M = m]$$
...
$$\tau_{it} = Y_{it}(\mathbf{d}_t) - Y_{it}(\mathbf{0}_t)$$-->

* Define $C_i := \displaystyle \sum_{t=2}^T D_{it}$ &mdash; the total number of periods that unit $i$ was treated

* Unit-specific average treatment effect
$$\bar{\tau}_i = \frac{1}{C_i} \sum_{t=2}^{T} D_{it} \big(Y_{it}(\mathbf{D}_{it}) - Y_{it}(\mathbf{0}_t) \big)$$
  This is the average treatment effect for unit $i$ in all the periods that it was treated

* Overall average treatment effect:
$$ATT^o := \E[\bar{\tau} | \mathbf{D} \neq \mathbf{0}_t]$$
...
$$ATT^{sum}(\sigma) := \E\Big[Y_{T}(\mathbf{D}) - Y_{T}(\mathbf{0}) \big| C=\sigma\Big]$$
...
\begin{align*}
  ATT^{sum}(\sigma, t) = \sum_{\mathbf{d}_t \in \mathcal{D}_t^\sigma} ATT(\mathbf{d}_t, t) \P(D_{t}=\mathbf{d}_t | \mathbf{D}_{t} \in \mathcal{D}_t^\sigma)
\end{align*}
...
# Treatment that can change amounts: back to the minimum wage example {visibility="uncounted"}
...
$$\frac{Y_{it}(\mathbf{d}_t) - Y_{it}(\mathbf{0}_t)}{d_t}$$
...
$$ATT^{scaled}(m,t) := \E\left[ \frac{Y_{t}(\mathbf{D}_{t}) - Y_{t}(\mathbf{0}_t)}{D_{t}} \Big| M = m \right]$$
...
1. Notice that above, we only invoked parallel trends with respect to untreated potential outcomes.
    ...
    * See @chaisemartin-dhaultfoeuille-pasquier-vazquez-2023 and @chaisemartin-dhaultfoeuille-2024 for approaches along these lines
...
2. Assumptions that [limit the "memory" of potential outcomes]{.alert} could be attractive in some applications
    ...
    * e.g., $Y_{t}(\mathbf{d}_t) = Y_{t}(\mathbf{d}_{t-5:t})$ &mdash; potential outcomes only depend on treatments in the last 5 periods
...
\begin{align*}
    \E[Y_{t}(d_{t-1},\mathbf{d}_{t-1}) - Y_{t-1}(\mathbf{d}_{t-1}) | \mathbf{D}_{t-1} = \mathbf{d}_{t-1})] = \E[Y_{t}(d_{t-1},\mathbf{d}_{t-1}) - Y_{t-1}(\mathbf{d}_{t-1}) | \mathbf{D}_{t} = (d_{t-1},\mathbf{d}_{t-1})]
\end{align*}
...
\begin{align*}
  ATT^{switchers}(\mathbf{d}_{t-1},t) &= \E[Y_{t}(1,\mathbf{d}_{t-1}) - Y_{t}(0,\mathbf{d}_{t-1}) | \mathbf{D}_{t} = (1,\mathbf{d}_{t-1})] \\
  &\overset{\textrm{PTA}}{=} \E[\Delta Y_{t} | \mathbf{D}_{t}=(1,\mathbf{d}_{t-1})] - \E[\Delta Y_{t} | \mathbf{D}_{t}=(0,\mathbf{d}_{t-1})]
\end{align*}
